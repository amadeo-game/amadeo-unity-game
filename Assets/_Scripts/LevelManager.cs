...
public class LevelManager : MonoBehaviour
{
    ...
    public void AdjustDifficultyBasedOnSessionData(SessionData sessionData)
    {
        ...
        if (sessionData.success)
        {
            ...
            sessionManager.SetTimeDuration(sessionManager.TimeDuration * 0.9f); // Decrease the time slightly for more challenge
        }
        else
        {
            var adjustedHeights = AdjustHeightsForFailure(sessionData.heights);
            sessionManager.SetHeights(adjustedHeights);
            sessionManager.SetTimeDuration(sessionManager.TimeDuration * 1.1f); // Increase the time slightly for less challenge
        }
    }
